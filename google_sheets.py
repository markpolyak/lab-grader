...
class GoogleSheet:
    ...
    def __get_multiple_sheets_data(self, dimension='COLUMNS'):
        ...
        :param dimension: passed to spreadsheet.values().batchGet as a value of
        majorDimension param. Possible values are 'COLUMNS' or 'ROWS'
        :returns: dict with sheet name as key and data as value
        """
        data = {}
        request = self.spreadsheet.values().batchGet(spreadsheetId=settings.google_spreadsheet_id, ranges=self.sheets,
    ...
    def __get_sheet_names(self):
        ...
        """
        sheets = []
        result = self.spreadsheet.get(spreadsheetId=settings.google_spreadsheet_id).execute()
    ...
    def set_student_github(self, student, dimension='COLUMNS', data_update=[]):
        """
        ...
        #     raise ValueError("GitHub account for student '{}' from group '{}' is '{}'. Can't set it to '{}'.
        #     Contact course staff if you want to update it.".format(student['name'], student['group'], student_github,
        #     student['github']))
        ...
        try:
            ...
            if other_student['group'] != student['group'] or other_student['name'] != student['name']:
                raise ValueError(
                    "Can't set GitHub account for student '{}' from group '{}' to '{}'. This GitHub account is"
                    " already used by student '{}' from group '{}'. Are you trying to cheat here?".format(
            ...
            # 2. does this student already have a github account?
                ...
                raise ValueError(
                    "GitHub account for student '{}' from group '{}' is '{}'. Can't set it to '{}'."
                    " Contact course staff if you want to update it.".format(
    ...
    def find_student(self, student, dimension='COLUMNS', searchby='name'):
        ...
        if student['group'] not in self.data:
            raise ValueError(
                "Group '{}' not found in spreadsheet! Available groups are: {}. Check your spelling or"
                " contact course staff if you don't see your group listed.".format(
        ...
        if 'name' in student and searchby == 'name':
            try:
                ...
                raise ValueError(
                    "Student '{}' not found in group {}! Check spelling or contact course staff if you are not on the"
                    " group list.".format(
                        student['name'], student['group']))
        elif 'github' in student:
            github_column = self._find_github_column(student, dimension)
            try:
                ...
                raise ValueError(
                    "Student with GitHub account {} not found in group {}!"
                    " Check spelling or contact course staff if you are not on the group list.".format(
    ...
    def _find_github_column(self, student, dimension='COLUMNS'):
        ...
        """
        github_column = None
        for i in range(0, len(self.data[student['group']])):
            if 'GitHub' in self.data[student['group']][i]:
                github_column = i
                break
        if github_column is None:
            raise ValueError(
                "Internal error! GitHub account column not found on sheet {}."
                " Please, verify spreadsheet integrity!".format(
                    student['group']))
    ...
    def set_student_lab_status(self, student, lab_id, value, dimension='COLUMNS', data_update=[]):
        """
    ...
    def get_student_task_id(self, student, dimension='COLUMNS'):
        ...
        if dimension != 'COLUMNS':
            raise ValueError("Not implemented! Only 'COLUMNS' dimension value is supported at the moment.")
        student_position = self.find_student(student, dimension=dimension)
        task_id = self.data[student['group']][TASK_ID_COLUMN][student_position]
        return task_id

    def batch_update(self, data_update):
        """
...
# this function hasn't been anywhere used
